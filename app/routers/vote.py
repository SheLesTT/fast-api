...
def vote(vote: schemas.Vote, db: Session = Depends(database.get_db),
    ...
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail = f"pst with id {id} does not exists ")
    ...
    if (vote.dir==1):
        if found_vote:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                                detail=f"user{current_user} has already posted on this vote ")
